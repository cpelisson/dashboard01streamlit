...
grafico_rec_vendedores = px.bar(
    df_vendedores.sort_values(by='sum', ascending=False).head(7),
    x = 'sum',
    y = df_vendedores.sort_values(by='sum', ascending=False).head(7).index,
    text_auto = True,
    title = 'Top 7 Vendedores por Receita'
)

grafico_vendas_vendedores = px.bar(
    df_vendedores.sort_values(by='count', ascending=False).head(7),
    x = 'count',
    y = df_vendedores.sort_values(by='count', ascending=False).head(7).index,
    text_auto = True,
